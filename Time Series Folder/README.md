...
![](Time-Series-Forecasting_files/figure-gfm/unnamed-chunk-8-1.png)<!-- -->
...
aes(day, forecasted, color="Florida (Univariate)"), size=1) +
geom_line(data = regional_alpha,
aes(day, forecasted, color=Region), size=1) +
geom_point(data = regional_alpha,
aes(day, actual, color=Region), size=1, alpha=0.2) +
theme_minimal() +
labs(x="Day", y="Hospital bed demand per case") +
theme(legend.title = element_blank(),legend.position="top", text = element_text(size=14))
```

![](Time-Series-Forecasting_files/figure-gfm/unnamed-chunk-12-1.png)<!-- -->
